...
export async function getOboToken(request: Request, scope: string): Promise<string> {
  ...
  if (!oboResult.ok) {
    ...
    throw new Error(`Tokenutveksling feilet for OBO token med følgende scope: ${scope}`);
  }
